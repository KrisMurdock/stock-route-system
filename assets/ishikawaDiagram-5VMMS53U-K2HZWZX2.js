...
var it = (function() {
  var e = /* @__PURE__ */ o(function(M, t, s, i) {
    ...
    return s;
  }, "o"), h = [1, 4], r = [1, 14], a = [1, 12], l = [1, 13], y = [6, 7, 8], f = [1, 20], d = [1, 18], w = [1, 19], u = [6, 7, 11], m = [1, 6, 13, 14], k = [1, 23], _ = [1, 24], x = [1, 6, 7, 11, 13, 14], N = {
    ...
    table: [{ 3: 1, 4: 2, 5: 3, 6: [1, 5], 8: h }, { 1: [3] }, { 1: [2, 1] }, { 4: 6, 6: [1, 7], 7: [1, 8], 8: h }, { 6: r, 7: [1, 10], 9: 9, 12: 11, 13: a, 14: l }, e(y, [2, 3]), { 1: [2, 2] }, e(y, [2, 4]), e(y, [2, 5]), { 1: [2, 6], 6: r, 12: 15, 13: a, 14: l }, { 6: r, 9: 16, 12: 11, 13: a, 14: l }, { 6: f, 7: d, 10: 17, 11: w }, e(u, [2, 18], { 14: [1, 21] }), e(u, [2, 16]), e(u, [2, 17]), { 6: f, 7: d, 10: 22, 11: w }, { 1: [2, 7], 6: r, 12: 15, 13: a, 14: l }, e(m, [2, 14], { 7: k, 11: _ }), e(x, [2, 8]), e(x, [2, 9]), e(x, [2, 10]), e(u, [2, 15]), e(m, [2, 13], { 7: k, 11: _ }), e(x, [2, 11]), e(x, [2, 12])],
    ...
    parse: /* @__PURE__ */ o(function(t) {
      var s = this, i = [0], c = [], p = [null], n = [], v = this.table, b = "", I = 0, S = 0, A = 0, L = 2, O = 1, X = n.slice.call(arguments, 1), g = Object.create(this.lexer), E = { yy: {} };
      for (var V in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, V) && (E.yy[V] = this.yy[V]);
      g.setInput(t, E.yy), E.yy.lexer = g, E.yy.parser = this, typeof g.yylloc > "u" && (g.yylloc = {});
      var R = g.yylloc;
      n.push(R);
      ...
      function st() {
        var P;
        return P = c.pop() || g.lex() || O, typeof P != "number" && (P instanceof Array && (c = P, P = c.pop()), P = s.symbols_[P] || P), P;
      }
      o(st, "lex");
      for (var T, K, W, B, Rt, q, F = {}, G, C, nt, U; ; ) {
        if (W = i[i.length - 1], this.defaultActions[W] ? B = this.defaultActions[W] : ((T === null || typeof T > "u") && (T = st()), B = v[W] && v[W][T]), typeof B > "u" || !B.length || !B[0]) {
          var Q = "";
          U = [];
          for (G in v[W])
            this.terminals_[G] && G > L && U.push("'" + this.terminals_[G] + "'");
          g.showPosition ? Q = "Parse error on line " + (I + 1) + `:
` + g.showPosition() + `
Expecting ` + U.join(", ") + ", got '" + (this.terminals_[T] || T) + "'" : Q = "Parse error on line " + (I + 1) + ": Unexpected " + (T == O ? "end of input" : "'" + (this.terminals_[T] || T) + "'"), this.parseError(Q, {
            text: g.match,
        ...
        if (B[0] instanceof Array && B.length > 1)
          throw new Error("Parse Error: multiple actions possible at state: " + W + ", token: " + T);
        switch (B[0]) {
          case 1:
            i.push(T), p.push(g.yytext), n.push(g.yylloc), i.push(B[1]), T = null, K ? (T = K, K = null) : (S = g.yyleng, b = g.yytext, I = g.yylineno, R = g.yylloc, A > 0 && A--);
            break;
          case 2:
            if (C = this.productions_[B[1]][1], F.$ = p[p.length - C], F._$ = {
              ...
              return q;
            C && (i = i.slice(0, -1 * C * 2), p = p.slice(0, -1 * C), n = n.slice(0, -1 * C)), i.push(this.productions_[B[1]][0]), p.push(F.$), n.push(F._$), nt = v[i[i.length - 2]][i[i.length - 1]], i.push(nt);
            break;
          case 3:
            return !0;
        }
...
it.parser = it;
var St = it, H, Et = (H = class {
  constructor() {
    this.stack = [], this.clear = this.clear.bind(this), this.addNode = this.addNode.bind(this), this.getRoot = this.getRoot.bind(this);
  }
  clear() {
    this.root = void 0, this.stack = [], this.baseLevel = void 0, lt();
  }
  getRoot() {
    return this.root;
  }
  addNode(h, r) {
    let a = at.sanitizeText(r, tt());
    if (!this.root) {
      this.root = { text: a, children: [] }, this.stack = [{ level: 0, node: this.root }], J(a);
      return;
    }
    ...
    for (l <= 0 && (l = 1); this.stack.length > 1 && this.stack[this.stack.length - 1].level >= l; )
      this.stack.pop();
    let y = this.stack[this.stack.length - 1].node, f = { text: a, children: [] };
...
}, o(H, "IshikawaDB"), H), $t = 14, j = 250, At = 30, It = 60, Lt = 5, _t = 82 * Math.PI / 180, gt = Math.cos(_t), kt = Math.sin(_t), mt = /* @__PURE__ */ o((e, h, r) => {
  let a = e.node().getBBox(), l = a.width + h * 2, y = a.height + h * 2;
  rt(e, y, l, r), e.attr("viewBox", `${a.x - h} ${a.y - h} ${l} ${y}`);
}, "applyPaddedViewBox"), Tt = /* @__PURE__ */ o((e, h, r, a) => {
  let y = a.db.getRoot();
  if (!y)
    return;
  let f = tt(), { look: d, handDrawnSeed: w, themeVariables: u } = f, m = ft(f.fontSize)[0] ?? $t, k = d === "handDrawn", _ = y.children ?? [], x = f.ishikawa?.diagramPadding ?? 20, N = f.ishikawa?.useMaxWidth ?? !1, D = yt(h), $ = D.append("g").attr("class", "ishikawa"), M = k ? pt.svg(D.node()) : void 0, t = M ? {
    roughSvg: M,
    seed: w ?? 0,
    lineColor: u?.lineColor ?? "#333",
    fillColor: u?.mainBkg ?? "#fff"
  } : void 0, s = `ishikawa-arrow-${h}`;
  k || $.append("defs").append("marker").attr("id", s).attr("viewBox", "0 0 10 10").attr("refX", 0).attr("refY", 5).attr("markerWidth", 6).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 10 0 L 0 5 L 10 10 Z").attr("class", "ishikawa-arrow");
  let i = 0, c = j, p = k ? void 0 : z($, i, c, i, c, "ishikawa-spine");
  if (Mt($, i, c, y.text, m, t), !_.length) {
    k && z($, i, c, i, c, "ishikawa-spine", t), mt(D, x, N);
    return;
  }
  i -= 20;
  let n = _.filter((g, E) => E % 2 === 0), v = _.filter((g, E) => E % 2 === 1), b = wt(n), I = wt(v), S = b.total + I.total, A = j, L = j;
  if (S > 0) {
    let g = j * 2, E = j * 0.3;
    A = Math.max(E, g * (b.total / S)), L = Math.max(E, g * (I.total / S));
  }
  let O = m * 2;
  A = Math.max(A, b.max * O), L = Math.max(L, I.max * O), c = Math.max(A, j), p && p.attr("y1", c).attr("y2", c), $.select(".ishikawa-head-group").attr("transform", `translate(0,${c})`);
  let X = Math.ceil(_.length / 2);
  for (let g = 0; g < X; g++) {
    let E = $.append("g").attr("class", "ishikawa-pair");
    for (let [V, R, Y] of [
      [_[g * 2], -1, A],
      [_[g * 2 + 1], 1, L]
    ])
      V && Nt(E, V, i, c, R, Y, m, t);
    i = E.selectAll("text").nodes().reduce((V, R) => Math.min(V, R.getBBox().x), 1 / 0);
  }
  if (k)
    z($, i, c, 0, c, "ishikawa-spine", t);
  else {
    p.attr("x1", i);
    let g = `url(#${s})`;
    $.selectAll("line.ishikawa-branch, line.ishikawa-sub-branch").attr("marker-start", g);
  }
  mt(D, x, N);
}, "draw"), wt = /* @__PURE__ */ o((e) => {
  let h = /* @__PURE__ */ o((r) => r.children.reduce((a, l) => a + 1 + h(l), 0), "countDescendants");
  return e.reduce(
    (r, a) => {
      let l = h(a);
...
}, "sideStats"), Mt = /* @__PURE__ */ o((e, h, r, a, l, y) => {
  let f = Math.max(6, Math.floor(110 / (l * 0.6))), d = e.append("g").attr("class", "ishikawa-head-group").attr("transform", `translate(${h},${r})`), w = Z(
    d,
    xt(a, f),
    0,
    0,
    ...
    "start",
    l
  ), u = w.node().getBBox(), m = Math.max(60, u.width + 6), k = Math.max(40, u.height * 2 + 40), _ = `M 0 ${-k / 2} L 0 ${k / 2} Q ${m * 2.4} 0 0 ${-k / 2} Z`;
  if (y) {
    let x = y.roughSvg.path(_, {
      roughness: 1.5,
      seed: y.seed,
      fill: y.fillColor,
      fillStyle: "hachure",
  ...
  w.attr("transform", `translate(${(m - u.width) / 2 - u.x + 3},${-u.y - u.height / 2})`);
}, "drawHead"), Pt = /* @__PURE__ */ o((e, h) => {
  let r = [], a = [], l = /* @__PURE__ */ o((y, f, d) => {
    let w = h === -1 ? [...y].reverse() : y;
    for (let u of w) {
      let m = r.length, k = u.children ?? [];
      r.push({
        depth: d,
        text: xt(u.text, 15),
        parentIndex: f,
        childCount: k.length
      }), d % 2 === 0 ? (a.push(m), k.length && l(k, m, d + 1)) : (k.length && l(k, m, d + 1), a.push(m));
    }
  }, "walk");
  return l(e, -1, 2), { entries: r, yOrder: a };
}, "flattenTree"), Bt = /* @__PURE__ */ o((e, h, r, a, l, y, f) => {
  let d = e.append("g").attr("class", "ishikawa-label-group"), u = Z(
    d,
    h,
    r,
    a + 11 * l,
...
}, "drawCauseLabel"), et = /* @__PURE__ */ o((e, h, r, a, l, y) => {
  let f = Math.sqrt(a * a + l * l);
  if (f === 0)
    return;
  let d = a / f, w = l / f, u = 6, m = -w * u, k = d * u, _ = h, x = r, N = `M ${_} ${x} L ${_ - d * u * 2 + m} ${x - w * u * 2 + k} L ${_ - d * u * 2 - m} ${x - w * u * 2 - k} Z`, D = y.roughSvg.path(N, {
    roughness: 1,
    seed: y.seed,
    fill: y.lineColor,
    fillStyle: "solid",
  ...
  let w = h.children ?? [], u = y * (w.length ? 1 : 0.2), m = -gt * u, k = kt * u * l, _ = r + m, x = a + k;
  if (z(e, r, a, _, x, "ishikawa-branch", d), d && et(e, r, a, r - _, a - x, d), Bt(e, h.text, _, x, l, f, d), !w.length)
    return;
  let { entries: N, yOrder: D } = Pt(w, l), $ = N.length, M = new Array($);
  for (let [p, n] of D.entries())
    M[n] = a + k * ((p + 1) / ($ + 1));
  ...
  let s = -gt, i = kt * l, c = l < 0 ? "ishikawa-label up" : "ishikawa-label down";
  for (let [p, n] of N.entries()) {
    let v = M[p], b = t.get(n.parentIndex), I = e.append("g").attr("class", "ishikawa-sub-group"), S = 0, A = 0, L = 0;
    if (n.depth % 2 === 0) {
      let O = b.y1 - b.y0;
      S = bt(b.x0, b.x1, O ? (v - b.y0) / O : 0.5), A = v, L = S - (n.childCount > 0 ? It + n.childCount * Lt : At), z(I, S, v, L, v, "ishikawa-sub-branch", d), d && et(I, S, v, 1, 0, d), Z(I, n.text, L, v, "ishikawa-label align", "end", f);
    } else {
      let O = b.childrenDrawn++;
      S = bt(b.x0, b.x1, (b.childCount - O) / (b.childCount + 1)), A = b.y0, L = S + s * ((v - A) / i), z(I, S, A, L, v, "ishikawa-sub-branch", d), d && et(I, S, A, S - L, A - v, d), Z(I, n.text, L, v, c, "end", f);
    }
...
}, "drawBranch"), Dt = /* @__PURE__ */ o((e) => e.split(/<br\s*\/?>|\n/), "splitLines"), xt = /* @__PURE__ */ o((e, h) => {
  if (e.length <= h)
    return e;
  let r = [];
  ...
  return r.join(`
`);
}, "wrapText"), Z = /* @__PURE__ */ o((e, h, r, a, l, y, f) => {
  let d = Dt(h), w = f * 1.05, u = e.append("text").attr("class", l).attr("text-anchor", y).attr("x", r).attr("y", a - (d.length - 1) * w / 2);
  for (let [m, k] of d.entries())
    u.append("tspan").attr("x", r).attr("dy", m === 0 ? 0 : w).text(k);
  return u;
}, "drawMultilineText"), bt = /* @__PURE__ */ o((e, h, r) => e + (h - e) * r, "lerp"), z = /* @__PURE__ */ o((e, h, r, a, l, y, f) => {
  if (f) {
    let d = f.roughSvg.line(h, r, a, l, {
      roughness: 1.5,
      seed: f.seed,
      stroke: f.lineColor,
      strokeWidth: 2
...
}, "drawLine"), Ot = { draw: Tt }, Ct = /* @__PURE__ */ o((e) => `
